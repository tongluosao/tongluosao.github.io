...
/*:
 * @plugindesc 用来处理画面效果的插件，所有画面效果相关的功能都在这里。
 * @author 冷血
 *
 *
 * 插件参数：
 *
 *   @param defaultBlackBack
 * 	 @desc 黑幕效果默认是开启还是关闭，写ON为开启，写OFF为关闭
 * 	 @default ON
 *
 *   @param blackBackWidth
 *   @desc 上下两个黑幕的宽度,不写按默认的设置
 *
 *   @param blackBackHeight
 *   @desc 上下两个黑幕的高度,不写按默认的设置
 *
 * @help
 *
 * 事件备注：
 *	 <keepColor>		#在画面黑白效果开启后保持颜色
 *
 * 插件命令：
 *   ScreenEffect BlackBack ON         	  # 黑幕效果开启，为避免遮挡尽可能将插件放在末尾
 *   ScreenEffect BlackBack OFF        	  # 黑幕效果关闭
 *   ScreenEffect RgbSplit  ON         	  # 色偏效果开启，可在ON后追加目标指定为画面时写0，为事件时写id，id写法为4000+id号,如4009指id编号为9的事件，还可追加强度
 *   ScreenEffect RgbSplit  OFF        	  # 色偏效果关闭, 指定画面写0，指定事件填写事件id(写法如上)
 *   ScreenEffect RgbSplit  SET  	   	  # 可在set之后追加目标，指定为画面时写0，为事件时写id(写法如上),还可追加强度
 *   ScreenEffect dropColor ON            # 画面黑白效果开启, 可在后面添加转变时间，单位帧。
 *   ScreenEffect dropColor OFF           # 画面黑白效果关闭, 可在后面添加转变时间，单位帧。
 *   ScreenEffect dropColorCharacter ON   # 玩家+事件黑白效果开启, 可在后面添加事件id(写法如色偏,玩家id为-1)，转变时间单位帧。
 *   ScreenEffect dropColorCharacter OFF  # 玩家+事件黑白效果关闭, 可在后面添加事件id(写法如色偏,玩家id为-1)，转变时间单位帧。
 *   ScreenEffect dropColorPlayer ON      # 玩家黑白效果开启, 可在后面添加转变时间，单位帧。
 *   ScreenEffect dropColorPlayer OFF     # 玩家黑白效果关闭, 可在后面添加转变时间，单位帧。
 */
(function() {
	var parameters = PluginManager.parameters('ScreenEffect');
	var defaultBlackBack = parameters['defaultBlackBack'] || '';
	var blackBackWidth = parseInt(parameters['blackBackWidth']);
	var blackBackHeight = parseInt(parameters['blackBackHeight']);

	var _Game_Screen_initialize = Game_Screen.prototype.initialize;
	Game_Screen.prototype.initialize = function() 
	{
	    _Game_Screen_initialize.call(this);
	    this._blackBackOn = defaultBlackBack == 'ON' ? true : false;
	    this._filterDatas = {};
	};

	Game_Screen.prototype.blackBackOn = function()
	{
		this._blackBackOn = true;
	}

	Game_Screen.prototype.blackBackOff = function()
	{
		this._blackBackOn = false;
	}

	Game_Screen.prototype.isBlackBackOn = function()
	{
		return this._blackBackOn;
	}

	Game_Screen.prototype.getFilterData = function(mapId)
	{
		var res = [];
		for(f in this._filterDatas)
		{
			if(this._filterDatas[f].mapId == mapId || this._filterDatas[f].idNum == 3999 || this._filterDatas[f].idNum == 0)
			{
				res.push(this._filterDatas[f]);
			}
		}
		return res;
	}

	Game_Screen.prototype.saveFilterData = function(mapId, idNum, effect, params)
	{
		var key = idNum == 0 || idNum == 3999 ? effect + '#' + idNum : mapId + '#' + effect + '#' + idNum;
		var value = {'mapId':mapId,'idNum':idNum,'effect':effect, 'params':params}
		this._filterDatas[key] = value;
	}

	Game_Screen.prototype.delFilterData = function(mapId, idNum, effect)
	{
		var key = idNum == 0 || idNum == 3999 ? effect + '#' + idNum : mapId + '#' + effect + '#' + idNum;
		//if(idNum == 3999) key = $gameMap.mapId() + '#' + idNum;
		delete this._filterDatas[key];
	}
    	...
    	if (command === 'ScreenEffect')
    	{
    		switch (args[0])
    		{
    			...
    			case 'BlackBack':
	    			if(args[1] === 'ON')
	            	{
	            		$gameScreen.blackBackOn();
	            		var scene = SceneManager._scene;
	            		if(scene && scene._spriteset && scene._spriteset._topBlackMask && scene._spriteset._bottomBlackMask)
	            		{
	            			scene._spriteset._topBlackMask.visible = true;
	            			scene._spriteset._bottomBlackMask.visible = true;
	            		}
	            	}
	            	if(args[1] === 'OFF')
	            	{
	            		$gameScreen.blackBackOff();
	            		var scene = SceneManager._scene;
	            		if(scene && scene._spriteset && scene._spriteset._topBlackMask && scene._spriteset._bottomBlackMask)
	            		{
	            			scene._spriteset._topBlackMask.visible = false;
	            			scene._spriteset._bottomBlackMask.visible = false;
	            		}
	            	}
	            	break;
	            //色偏效果
	            case 'RgbSplit':
	            	if(args[1] === 'ON')
	            	{
	            		...
	            		s.rgbSplitOn(args[2], args[3]);
	            	}
	            	if(args[1] === 'OFF')
	            	{
	            		var s = SceneManager._scene._spriteset;
	            		s.rgbSplitOff(args[2]);
	            	}
	            	if(args[1] === 'SET')
	            	{
	            		var s = SceneManager._scene._spriteset;
	            		s.rgbSplitSet(args[2], args[3]);
	            	}
	            	break;
	            case 'dropColor':
		            if(args[1] === 'ON')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOn(0, parseInt(args[2]));
		            }
		            if(args[1] === 'OFF')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOff(0, parseInt(args[2]));		            	
		            }
		            break;
	            case 'dropColorCharacter':
		            if(args[1] === 'ON')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOn(args[2], parseInt(args[3]));
		            }
		            if(args[1] === 'OFF')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOff(args[2], parseInt(args[3]));		            	
		            }
		            break;
	            case 'dropColorPlayer':
		            if(args[1] === 'ON')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOn(3999, parseInt(args[2]));
		            }
		            if(args[1] === 'OFF')
		            {
		            	var s = SceneManager._scene._spriteset;
		            	s.dropColorOff(3999, parseInt(args[2]));		            	
		            }
		            break;
    		}
    	}
	...
	var _Spriteset_Base_createScreenSprites = Spriteset_Base.prototype.createScreenSprites;
	...
	Spriteset_Map.prototype.createPictures = function() 
	{
		...
		Spriteset_Base.prototype.createPictures.apply(this);
		_Spriteset_Base_createScreenSprites.call(this);
	}

	//创建黑幕
	Spriteset_Map.prototype.createBlackMask = function() 
	{
		//参数定义
		...
		var x = 0;
		var y = 0;
		var w = blackBackWidth || Graphics.width;
		var h = blackBackHeight || parseInt(Graphics.height * 0.25);
	    w = w + blurFilter.padding * 2;
	    h = h + blurFilter.padding * 2;
	    var padding = 50;		

		//上层黑色遮罩
	    this._topBlackMask = new Sprite();
	    this._topBlackMask.bitmap = new Bitmap(w, h);
	    var bitmap = this._topBlackMask.bitmap;
	    bitmap.fillRect(x, y, w, h - padding, 'rgba(0, 0, 0, 1)');
	    bitmap.gradientFillRect(x, h - padding, w, padding, 'rgba(0, 0, 0, 1)', 'rgba(0, 0, 0, 0)', true);
	    bitmap.gradientFillRect(x, h - padding, w, padding, 'rgba(0, 0, 0, 1)', 'rgba(0, 0, 0, 0)', true);
	    this._topBlackMask.x -= blurFilter.padding;
	    this._topBlackMask.y -= blurFilter.padding;
	    this._topBlackMask.filters = [blurFilter];

	    //下层黑色遮罩
	    //x = 0;
	    //y = 0;
	    //w = Graphics.width + blurFilter.padding * 2;
	    //h = parseInt(Graphics.height * 0.25) + blurFilter.padding * 2;
	    //padding = 50;
	    this._bottomBlackMask = new Sprite();
	    this._bottomBlackMask.y = Graphics.height - h;
	    this._bottomBlackMask.bitmap = new Bitmap(w, h);
	    bitmap = this._bottomBlackMask.bitmap;
	    bitmap.fillRect(x, padding, w, h - padding, 'rgba(0, 0, 0, 1)');
	    bitmap.gradientFillRect(x, y, w, padding, 'rgba(0, 0, 0, 0)', 'rgba(0, 0, 0, 1)', true);
	    bitmap.gradientFillRect(x, y, w, padding, 'rgba(0, 0, 0, 0)', 'rgba(0, 0, 0, 1)', true);
	    this._bottomBlackMask.x -= blurFilter.padding;
	    this._bottomBlackMask.y += blurFilter.padding;
	    this._bottomBlackMask.filters = [blurFilter];

	    //添加精灵
	    this.addChild(this._topBlackMask);
	    this.addChild(this._bottomBlackMask);

	    //是否显示
	    if($gameScreen.isBlackBackOn())
	    {
	    	this._topBlackMask.visible = true;
	    	this._bottomBlackMask.visible = true;
	    }
	    else
	    {
	    	this._topBlackMask.visible = false;
	    	this._bottomBlackMask.visible = false;
	    }
	};

	Spriteset_Map.prototype.getApplyTarget = function(idNum, effect)
	{
		var s = SceneManager._scene._spriteset;
		var targets = [];
		...
		if(idNum == 0)
		{
			if(effect == 'RgbSplit')
			{
				targets.push(s._baseSprite);
			}
			else if(effect == 'dropColor')
			{
				var filterArea = new Rectangle(0, 0, Graphics.width, Graphics.height-1);
				//远景
				this._parallax.filterArea = filterArea;
				targets.push(this._parallax);
				//地图
				if (Graphics.isWebGL())
				{
					this._tilemap.lowerZLayer.filterArea = filterArea;
					targets.push(this._tilemap.lowerZLayer);
					this._tilemap.upperZLayer.filterArea = filterArea;
					targets.push(this._tilemap.upperZLayer);
				}
				else
				{
					this._tilemap._lowerLayer.filterArea = filterArea;
					targets.push(this._tilemap._lowerLayer);
					this._tilemap._upperLayer.filterArea = filterArea;
					targets.push(this._tilemap._upperLayer);
				}
				//玩家和事件
				var chars = this._characterSprites;
				for(var i = 0;i < chars.length;i++)
				{
					if(chars[i]._character instanceof Game_Event)
					{
						var keepColor = chars[i]._character.event().meta.keepColor;
						// if(!chars[i].visible) continue;
						// if(chars[i].opacity <= 0) continue;
						// if(chars[i]._characterName == '') continue;
		...
		else
		{
			...
			else if(idNum > 4000)
			{
				...
				{
					gameObj = $gameMap.event(idNum);
				}
			}
			if(gameObj != null)
			{
				var chars = s._characterSprites;
				for(var i = 0;i < chars.length;i++)
				{
					if(chars[i]._character == gameObj)
					{
						targets.push(chars[i]);
						break;
					}
	...
	Spriteset_Map.prototype.getRGBSplitFilter = function(strength)
	{
		strength = strength || -3;
	...
	Spriteset_Map.prototype.rgbSplitOn = function(idNum, strength)
	{
		idNum = idNum || 0;
		var targets = this.getApplyTarget(idNum, 'RgbSplit');
		if(targets && targets.length > 0)
		{
			for(var i = 0;i < targets.length;i++)
			{
				targets[i]._filters = targets[i]._filters || [];
				targets[i]._filters.push(this.getRGBSplitFilter(strength));
			}
			$gameScreen.saveFilterData($gameMap.mapId(),idNum,'RgbSplit',[strength]);
		}
	}

	Spriteset_Map.prototype.rgbSplitOff = function(idNum)
	{
		idNum = idNum || 0;
		var targets = this.getApplyTarget(idNum, 'RgbSplit');
		if(targets && targets.length > 0)
		{
			for(var i = 0;i < targets.length;i++)
			{
				...
				var fs = target._filters;
				if(fs && fs.length > 0)
				{
					for(var j = 0;j < fs.length;j++)
					{
						if(fs[j] instanceof PIXI.filters.RGBSplitFilter)
						{
							fs.splice(j,1);
							break;
						}
			...
			$gameScreen.delFilterData($gameMap.mapId(),idNum,'RgbSplit');
		}
	}

	Spriteset_Map.prototype.rgbSplitSet = function(idNum, strength)
	{
		this.rgbSplitOff(idNum);
		this.rgbSplitOn(idNum, strength);
	}

	Spriteset_Map.prototype.dropColorOn = function(idNum,time)
	{
		time = time || 1;
		this._dropColorQueue = this._dropColorQueue || [];
		var targets = this.getApplyTarget(idNum, 'dropColor');
		...
		if(targets && targets.length > 0)
		{
			...
			f.saturation = 1;
			// this._dropOnTime = time;
			// this._dropFilter = f;
			for(var i = 0;i < targets.length;i++)
			{
				targets[i]._filters = targets[i]._filters || [];
				targets[i]._filters.push(f);
			}
			this._dropColorQueue.push({'dropTime':time, 'dropFilter':f, 'targets':targets, 'switch':'on'})
			$gameScreen.saveFilterData($gameMap.mapId(),idNum,'dropColor',[]);	
		}
	}

	Spriteset_Map.prototype.dropColorOff = function(idNum,time)
	{
		time = time || 1;
		this._dropColorQueue = this._dropColorQueue || [];
		var targets = this.getApplyTarget(idNum, 'dropColor');
		// this._dropOffTime = time;
		// this._dropFilter = null;
		if(targets && targets.length > 0)
		{
			var f = null;
			...
			if(fs && fs.length > 0)
			{
				...
				{
					if(fs[i] instanceof PIXI.filters.AdjustmentFilter)
					{
						f = fs[i];
						break;
					}
				}
				if(f != null) {this._dropColorQueue.push({'dropTime':time, 'dropFilter':f, 'targets':targets, 'switch':'off'})}
				$gameScreen.delFilterData($gameMap.mapId(),idNum,'dropColor');
			}
		}
	}

	Spriteset_Map.prototype.updateDropColor = function()
	{
		if(this._dropColorQueue && this._dropColorQueue.length > 0)
		{
			for(var i = 0;i < this._dropColorQueue.length;i++)
			{
				var q = this._dropColorQueue[i];
				if(q && q.dropTime > 0 && q.dropFilter)
				{
					if(q.switch == 'on')
					{
						var d = q.dropTime;
						q.dropFilter.saturation = (q.dropFilter.saturation * (d - 1) + 0) / d;
						q.dropTime--;
					}
					else if(q.switch == 'off')
					{
						var d = q.dropTime;
						q.dropFilter.saturation = (q.dropFilter.saturation * (d - 1) + 1) / d;
						q.dropTime--;
					}
					...
					if(q.dropTime <= 0)
					{
						if(q.switch == 'off')
						{
							...
							if(targets && targets.length > 0)
							{
								for(var j = 0;j < targets.length;j++)
								{
									...
									{
										if(fs[k] instanceof PIXI.filters.AdjustmentFilter)
										{
											fs.splice(k,1);
											break;
										}
						...
						this._dropColorQueue.splice(i,1);
					}
				}
				else
				{
		...
		// if(this._dropOnTime > 0 && this._dropFilter)
		// {
		// 	var d = this._dropOnTime;
		// 	this._dropFilter.saturation = (this._dropFilter.saturation * (d - 1) + 0) / d;
		// 	this._dropOnTime--;
		// }
		// if(this._dropOffTime > 0 && this._dropFilter)
		// {
		// 	var d = this._dropOffTime;
		// 	this._dropFilter.saturation = (this._dropFilter.saturation * (d - 1) + 1) / d;
		// 	this._dropOffTime--;
		// 	if(this._dropOffTime <= 0 && this._dropTargets && this._dropTargets.length > 0)
		// 	{
		// 		var targets = this._dropTargets;
		// 		for(var i = 0;i < targets.length;i++)
		// 		{
		// 			targets[i]._filters = [];
		// 		}
    ...
    Scene_Map.prototype.start = function() 
    {
        ...
        //处理场景转换后过滤器的挂载
    	var res = $gameScreen.getFilterData($gameMap.mapId());
    	console.log('保存的对象');
    	console.log(res);
    	console.log('----------');
    	if(res && res.length > 0)
    	{
    		...
    		{
    			if(res[i].effect == 'dropColor')
    			{
    				this._spriteset.dropColorOn(res[i].idNum,1);
    			}
    			else if(res[i].effect == 'RgbSplit')
    			{
    				this._spriteset.rgbSplitOn(res[i].idNum,res[i].params[0]);
    			}
    		}
    	}
    };
