...
Window_Base.prototype.processEscapeCharacter = function(code, textState) {
  ...
  case 'PO':
	var id = this.obtainEscapeParam(textState);
    var nextSpace = textState.text.length + 1;
    var nextBreak = textState.text.indexOf('\n', textState.index + 1);
    var nextCenter = textState.text.indexOf('\x1bPO', textState.index + 1);
    if (nextBreak > 0) nextSpace = Math.min(nextSpace, nextBreak);
    if (nextCenter > 0) nextSpace = Math.min(nextSpace, nextCenter);
    var word = textState.text.substring(textState.index, nextSpace);
    var size = this.textWidthExCheck(word);
    var x = 0;
    if(id == 1){
      x = (this.wordwrapWidth() - size)/2;
    }else if(id == 2){
      x = (this.wordwrapWidth() - size);
    }
    if(x > 0) textState.x = x;
	break;
  default:
