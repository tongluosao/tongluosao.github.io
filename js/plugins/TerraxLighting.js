				...
				if ($gameVariables.GetScriptActive() == true && $gameMap.mapId() >= 0) {
					...
					if (darkenscreen == true) {
						...
						if (SceneManager._scene._spriteset) {
							...
							var playerSprite = sprites[sprites.length - 1];
							if (playerSprite._character !== $gamePlayer) {
								playerSprite = sprites.filter(function(s){return s._character === $gamePlayer;})[0];
							}
							y1 = y1 + (ph / 2) - Math.floor(playerSprite.height / 2);
						}
						////////////////
